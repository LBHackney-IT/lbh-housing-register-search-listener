...
#
resource "aws_sqs_queue" "housingregistersearch_dead_letter_queue" {
...
#
resource "aws_sqs_queue" "housingregistersearch_queue" {
...
# resource "aws_sqs_queue_policy" "housingregistersearch_queue_policy" {
  ...
  policy = <<POLICY
  {
      ...
      "Statement": [
      {
          ...
          "Resource": "${aws_sqs_queue.housingregistersearch_queue.arn}",
          "Condition": {
          "ArnEquals": {
...
# 
resource "aws_sns_topic_subscription" "housingregistersearch_queue_subscribe_to_housingregister_sns" {
  topic_arn = data.aws_ssm_parameter.housingregister_sns_topic_arn.value
  protocol  = "sqs"
  endpoint  = aws_sqs_queue.housingregistersearch_queue.arn
...
# 
resource "aws_ssm_parameter" "housingregistersearch_sqs_queue_arn" {
  name  = "/sqs-queue/development/housingregistersearch/arn"
  type  = "String"
  value = aws_sqs_queue.housingregistersearch_queue.arn
}
